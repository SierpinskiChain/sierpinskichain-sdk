...
export function listenForWalletResponses() {
    if (typeof window === "undefined")
        return;
    const handler = (event) => {
...
// ── Core connection ─────────────────────────────────────────────────────────
export function connectToTriWallet(triWalletUrl, method, params = [], origin) {
...
// ── Convenience helpers ─────────────────────────────────────────────────────
export function connectTriWallet(walletUrl = TRIWALLET_URL) {
    return connectToTriWallet(walletUrl, "spc_requestAccounts");
}
export function signMessageWithTriWallet(message, walletUrl = TRIWALLET_URL) {
    return connectToTriWallet(walletUrl, "spc_signMessage", [message]);
}
export function sendTransactionViaTriWallet(params, walletUrl = TRIWALLET_URL) {
